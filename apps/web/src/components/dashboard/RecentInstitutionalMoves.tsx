...
import { useInstitutionalMoves } from "@/hooks/useInstitutionalMoves";

export function RecentInstitutionalMoves() {
  const { data, isLoading, isUsingFallback } = useInstitutionalMoves();
    ...
    <GlassPanel>
      ...
      <table className="w-full border-collapse text-left">
        ...
        <tbody className="divide-y divide-[#424754]/20 font-mono text-[14px] text-[#e0e2ed]">
          {data.map((move, index) => (
            <tr
              key={`${move.institution}-${move.ticker}-${move.time}-${move.value}-${index}`}
